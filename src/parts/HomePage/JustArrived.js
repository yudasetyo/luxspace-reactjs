...
function Loading() {
  return Array(6)
    ...
    .map((_, index) => {
      return (
        <div className="card group relative px-4" key={index}>
          <div
            className="card-shadow relative overflow-hidden rounded-xl bg-gray-300"
            style={{ width: 287, height: 386 }}
          ></div>
          <div className="mt-3 h-3 w-24 rounded-full bg-gray-300"></div>
          <div className="mt-2 h-3 w-36 rounded-full bg-gray-300"></div>
        </div>
      );
    });
}
  ...
  return (
    <section className="flex flex-col py-16">
      ...
      <div className="overflow-x-hidden px-4" id="carousel">
        ...
        {isLoading ? (
          <div
            className="relative -mx-4 flex flex-row"
            style={{
              paddingLeft:
          ...
          <Carousel refContainer={refContainer}>
            {data.data.map((item) => {
              return (
                <div className="card group relative px-4" key={item.id}>
                  <div
                    className="card-shadow relative overflow-hidden rounded-xl"
                    style={{ width: 287, height: 386 }}
                  >
                    <div className="absolute flex h-full w-full items-center justify-center bg-black bg-opacity-50 opacity-0 transition duration-200 group-hover:opacity-100">
